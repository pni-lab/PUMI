...
from PUMI.engine import FuncPipeline, QcPipeline
...
@QcPipeline(
    inputspec_fields=['func', 'atlas', 'confounds'],
    outputspec_fields=[]
)
def fMRI2QC(wf, tag=None, indiv_atlas=False):
    """

    Adapted from https://github.com/spisakt/PUMI/blob/master/utils/QC.py
    ...
    if wf.get_node('inputspec').inputs.atlas is None:
        wf.get_node('inputspec').inputs.atlas = os.environ['FSLDIR'] + '/data/atlases/HarvardOxford/HarvardOxford-cort-maxprob-thr25-3mm.nii.gz'
    ...
    wf.connect('inputspec', 'func', plotfmri, 'func')
    wf.connect('inputspec', 'atlas', plotfmri, 'atlaslabels')
    wf.connect('inputspec', 'confounds', plotfmri, 'confounds')

    if tag:
        wf.connect(plotfmri, 'plotfile', 'sinker', 'fMRI2QC/' + tag)
    else:
        wf.connect(plotfmri, 'plotfile', 'sinker', 'fMRI2QC/' + wf.name)
...
def plot_fmri_qc(func, atlaslabels, confounds, output_file=None):
    ...
    def plot_carpet(img, atlaslabels, detrend=True, nskip=0, size=(4000, 3000),
                    ...
                    lut=None):
        """
        Adapted from https://github.com/poldracklab/niworkflows and https://github.com/spisakt/PUMI
        ...
        # actually load data
        img = nb.load(img)
        atlaslabels = nb.load(atlaslabels).get_fdata()
    ...
    def calculate_DVARS(rest):
        ...
        rest_data = nib.load(rest).get_fdata().astype(np.float32)

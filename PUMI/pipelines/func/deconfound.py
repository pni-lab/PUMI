...
@QcPipeline(inputspec_fields=['func', 'motion_correction', 'plot_motion_trans', 'FD_figure'],
            ...
            regexp_sub=[(r'(.*\/)([^\/]+)\/([^\/]+)\/([^\/]+)$', r'\g<1>motion_correction/\g<3>-\g<2>.png'),
                        ('_subject_', 'sub-')])
...
def motion_correction_mcflirt(wf, reference_vol='middle', FD_mode='Power', **kwargs):
    ...
    Parameters
    ----------
    reference_vol (str): Either "first", "middle", "last", "mean", or the index of the volume which the rigid body registration (motion correction) will use as reference. Default 'middle'.
    FD_mode: Either "Power" or "Jenkinson"

    Inputs
    ----------
    in_file (str): The reoriented functional file.
